...
import { Calendar, Filter, Plus, User, X } from "lucide-react";
...
type BacklogSearchParams = { taskId?: string };

export const Route = createFileRoute(
  "/_layout/_authenticated/dashboard/workspace/$workspaceId/all-tasks/backlog",
)({
...
function RouteComponent() {
  ...
  const clearFilters = () => {
    setFilters({ priority: null, assignee: null, dueDate: null, labels: [] });
  };
  ...
  const filteredProject = useMemo(() => {
    ...
    return {
      ...local,
      plannedTasks: filterTasks(local.plannedTasks || []),
      archivedTasks: filterTasks(local.archivedTasks || []),
    };
  }, [local, filters, getTaskLabels]);
  ...
  return (
    <AllTasksLayout workspaceId={workspaceId} activeView="backlog">
      <PageTitle
      ...
      <div className="relative flex flex-col h-full min-h-0 overflow-hidden">
        ...
        <TaskDetailsSheet
          taskId={taskId}
          projectId={
            sortedProject?.plannedTasks?.find((tk) => tk.id === taskId)
              ?.projectId ??
            sortedProject?.archivedTasks?.find((tk) => tk.id === taskId)
              ?.projectId ??
            ""
          }
